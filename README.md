...
Elevation ($e$): Determines the physical landscape, from abyssal oceanic trenches to the highest heavenly peaks.
Moisture ($m$): A humidity layer that dictates the difference between the golden Dune Seas and the deep, canopied forests.
Qi Intensity ($q$): Represents the spiritual flux of the land. High $q$ values create Jade Forests or Volcanoes, while low $q$ values result in more mundane plains.
✨ Key Features
...
🚀 Deployment
Download the xianxia-map.html file.
Open it in any modern web browser.
No server, build-tools, or external assets required.
